...
annotateOffTargets <- function(scores, txdb, orgAnn, ignore.strand = TRUE) {
  score.RD <- GRanges(seqnames = Rle(scores$chrom), ranges = IRanges(start = scores$chromStart, end = scores$chromEnd, names = scores$forViewInUCSC), strand = scores$strand)
  ...
  if (ignore.strand) {
    score.plus.RD <- GRanges(seqnames = Rle(scores$chrom), ranges = IRanges(start = scores$chromStart, end = scores$chromEnd, names = scores$forViewInUCSC), strand = "+")
    score.minus.RD <- GRanges(seqnames = Rle(scores$chrom), ranges = IRanges(start = scores$chromStart, end = scores$chromEnd, names = scores$forViewInUCSC), strand = "-")
    score.list <- GRangesList(score.plus.RD, score.minus.RD)
    score.RD <- c(score.list[[1]], score.list[[2]])
    scores <- rbind(scores, scores)
  } 
  
  ann.scores <- overlapsAny(score.RD, allExons, minoverlap = 1L, type = "any", ignore.strand=FALSE)
  inExon <- cbind(forViewInUCSC = names(score.RD), inExon = unlist(ann.scores))
  ...
  if (length(queryHits(overlapGenes)) > 0 && !is.null(orgAnn) && class(orgAnn) == "AnnDbBimap") {
  ...
  if (length(grep("FBgn", entrez_id[1])) > 0 && !is.null(orgAnn) && class(orgAnn) == "AnnDbBimap") {
  ...
  return(unique(data.frame(scores)))
}
