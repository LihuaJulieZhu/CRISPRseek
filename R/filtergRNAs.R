...
#' @export
filtergRNAs <- function(all.gRNAs = NULL, 
                        pairOutputFile = NULL, 
  ...
  if (is.null(REpatternFile)) {
    stop("REpatternFile containing the restriction enzyme cut pattern is required!")
  }
  ...
  patterns <- unique(readDNAStringSet(REpatternFile, format,use.names = TRUE))
  ...
  gRNAs.RE <- data.frame(gRNAPlusPAM = "", REcutgRNAName = "", REname = "", REpattern = "", REcutStart = "", REcutEnd = "")
  ...
  for (j in 1:length(patterns)) {
    ...
    if (revpattern != pattern) {
      ...
      minus.gRNAs <- do.call(rbind, lapply(1:length(all.gRNAs), function(i) {
        ...
        do.call(rbind, lapply(1:length(res1), function(k) {
          if (res1[k] > 0 && 
            ...
            c(as.character(seqs[i]),seq.names[i], pattern.name, as.character(reverseComplement(patterns[[j]])), this.pattern.size - 1 + res1[k], res1[k])
          } else if (res1[k] >0 && 
                     overlap.allpos && 
                     min.pStart.plus >= res1[k] && 
                     max.pStart.plus <= (res1[k] + this.pattern.size -1)) {
      ...
      if (length(minus.gRNAs) >1) {
        colnames(minus.gRNAs) <- c("gRNAPlusPAM", "REcutgRNAName", "REname", "REpattern", "REcutStart","REcutEnd")
        gRNAs.RE <- rbind(gRNAs.RE,  minus.gRNAs)	
      }
    }
    
    pattern <- translatePattern(pattern)
    pos.gRNAs <- do.call(rbind, lapply(1:length(all.gRNAs), function(i) {
      ...
      do.call(rbind, lapply(1:length(res1), function(k) {
        if (res1[k] > 0 && 
            ...
            ((min.pStart.plus >= res1[k] && 
             ...
             (max.pStart.plus >= res1[k] && 
              max.pStart.plus <= (res1[k] + this.pattern.size -1)))) {
          c(as.character(seqs[i]), seq.names[i], pattern.name, as.character(patterns[[j]]), res1[k], res1[k] + this.pattern.size - 1)
        } else if (res1[k] > 0 && 
                   overlap.allpos && 
                   min.pStart.plus >= res1[k] && 
                   max.pStart.plus <= (res1[k] + this.pattern.size -1)) {
    ...
    if (length(pos.gRNAs) > 0) {
      colnames(pos.gRNAs) <- c("gRNAPlusPAM", "REcutgRNAName", "REname", "REpattern", "REcutStart", "REcutEnd")
      gRNAs.RE <- rbind(gRNAs.RE,  pos.gRNAs)
    }
  }
  
  gRNAs.RE <- gRNAs.RE[gRNAs.RE[,1] != "", ]
  if (!is.null(pairOutputFile) && file.exists(pairOutputFile)) {
    gRNAs.RE.plus <- gRNAs.RE
    gRNAs.RE.minus <- gRNAs.RE
    colnames(gRNAs.RE.plus) <- paste("Forward", colnames(gRNAs.RE.plus), sep = "")
    colnames(gRNAs.RE.minus) <- paste("Reverse", colnames(gRNAs.RE.minus), sep = "")
    pairgRNAs <- read.xlsx(pairOutputFile)
    ann.gRNAs <- merge(pairgRNAs, gRNAs.RE.plus, by = "ForwardgRNAPlusPAM", all.x = TRUE)
    ann.gRNAs <- merge(ann.gRNAs, gRNAs.RE.minus, by = "ReversegRNAPlusPAM", all.x = TRUE)
    ann.gRNAs <- cbind(ann.gRNAs[,1], ann.gRNAs[,3], ann.gRNAs[,2], ann.gRNAs[,4:dim(ann.gRNAs)[2]])
    colnames(ann.gRNAs)[1:3] <- c("ReversegRNAPlusPAM", "ReversegRNAName", "ForwardgRNAPlusPAM")
    ann.gRNAs <- ann.gRNAs[order(ann.gRNAs[,1]), ]
    withRE <- ann.gRNAs[!is.na(ann.gRNAs$ForwardREname) | !is.na(ann.gRNAs$ReverseREname), ]
    ...
    gRNAs  <- DNAStringSet(c(as.character(withRE$ForwardgRNAPlusPAM), as.character(withRE$ReversegRNAPlusPAM)))
    names(gRNAs) <- c(as.character(withRE$ForwardgRNAName), as.character(withRE$ReversegRNAName))
  } else {
    if (dim(gRNAs.RE)[1] == 0 && findgRNAsWithREcutOnly) {
      stop("No gRNAs with RE sites!")
    }
          
    temp <- unique(cbind(as.character(gRNAs.RE$gRNAPlusPAM), as.character(gRNAs.RE$REcutgRNAName)))
