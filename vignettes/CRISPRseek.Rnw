...
@
\end{scriptsize}
\subsection{Scenario 7. Find potential gRNAs preferentially targeting one of two alleles without running time-consuming off-target analysis on all possible gRNAs.}
...
2013. 31:827-834. 
\bibitem[Lihua Julie Zhu, Benjamin R. Holmes, Neil Aronin and Michael Brodsky]{Zhu et al}Lihua Julie Zhu, Benjamin R. Holmes, Neil Aronin and Michael Brodsky. CRISPRseek: A Bioconductor package to help with the design of guide RNAs in CRISPR-Cas9 systems for targeted genome editing. (In preparation)
\end{thebibliography}
