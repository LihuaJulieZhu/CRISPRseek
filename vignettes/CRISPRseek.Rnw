...
and enable.multicore = TRUE will improve the performance. We also suggest split 
the super long sequence into smaller chunks and perform offTarget analysis for
each subsequence separately (Thank Alex Williams for sharing this use case at
...
results <- offTargetAnalysis(inputFilePath, annotatePaired = FALSE, 
    chromToSearch = "chrX",
    enable.multicore = TRUE, annotateExon = FALSE,
