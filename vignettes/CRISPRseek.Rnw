...
without off-target analysis. Paramter rule.set can be set to 
...
To use the rule set 2, first install python 2.7, then install the python packages: scikit-learn 0.16.1, pickle, pandas, numpy nd scipy.
