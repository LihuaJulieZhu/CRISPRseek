...
first target site. Currently, only the Lindel method by Chen et al., 2019 is available. 
To use this feature, users need to install python package Lindel first.
Please type help(predictRelativeFreqIndels) for more details.
